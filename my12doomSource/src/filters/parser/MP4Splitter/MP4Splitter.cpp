...
HRESULT CMP4SplitterFilter::CreateOutputs(IAsyncReader *pAsyncReader)
{
	...
	{
		for(AP4_List<AP4_Track>::Item *item = movie->GetTracks().FirstItem(); item;	item = item->GetNext())
		{
			...
			else if(AP4_Hvc1SampleEntry *hvc1 = dynamic_cast<AP4_Hvc1SampleEntry *>(track->GetTrakAtom()->FindChild("mdia/minf/stbl/stsd/hvc1")))
			{
				if(AP4_hvcCAtom *hvcC = dynamic_cast<AP4_hvcCAtom *>(hvc1->GetChild(AP4_ATOM_TYPE_hvcC)))
				{
					...
					memset(vih, 0, mt.FormatLength());
					vih->hdr.bmiHeader.biSize = sizeof(vih->hdr.bmiHeader);
					vih->hdr.bmiHeader.biWidth = (LONG)hvc1->GetWidth();
					vih->hdr.bmiHeader.biHeight = (LONG)hvc1->GetHeight();
					vih->hdr.bmiHeader.biCompression = '01MH';
					vih->hdr.bmiHeader.biPlanes = 1;
					vih->hdr.bmiHeader.biBitCount = 24;
					vih->hdr.dwPictAspectRatioX = vih->hdr.bmiHeader.biWidth;
					vih->hdr.dwPictAspectRatioY = vih->hdr.bmiHeader.biHeight;
					...
					for(int i = 0; i < xps_count; i++)
					{
						BYTE type = *src++;
						int nal_count = (src[0] << 8) + src[1];				// assume = 1
						src +=2;
						int size = (src[0] << 8) + src[1];
						src +=2;
