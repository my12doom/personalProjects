...
CRMFile::CRMFile(IAsyncReader* pAsyncReader, HRESULT& hr)
	: CBaseSplitterFile(pAsyncReader, hr, 16*1024)
{
	if(FAILED(hr)) {
		return;
	}
...
HRESULT CRealVideoDecoder::CheckInputType(const CMediaType* mtIn)
{
	...
	if(!m_pInput->IsConnected()) {
		...
		GetModuleFileName((HINSTANCE)&__ImageBase, buff, countof(buff));
		for(int i=_tcslen(buff) -1; i>0; i--)
			if (buff[i] == _T('\\'))
			{
				buff[i+1] = NULL;
				break;
		...
		paths.AddTail(axpath + olddll);
		paths.AddTail(axpath + newdll);
