	...
	else
    {
        ...
        if(s.type!=0xff)
        {
            ...
            if(s.frame_num)
            {
				...
				if (demux_target2 && (pid == pid_to_demux2 || s.type == type_todemux2))
					demux_target2->insert(len, (BYTE*)ptr);
...
//my12doom
bool ts::demuxer::is_ts(const char * name)
{
	// detect stream type;
	char buf[192];
	DWORD read;
...
int ts::demuxer::fast_scan_file(const char* name, int scan_packet_count/*=250000*/)
{
	// some init
	prefix.clear();
