...
namespace ts
{
    ...
    public:
		...
		int type_todemux;
		int pid_to_demux;
		CFileBuffer *demux_target;
		int type_todemux2;
		int pid_to_demux2;
		CFileBuffer *demux_target2;
		bool silence;
		bool is_ts(const char * name);
		int fast_scan_file(const char* name, int scan_packet_count=250000);
...
#endif
    public:
        demuxer(void):hdmv(false),av_only(true),parse_only(false),dump(0),channel(0),base_pts(0),pes_output(0),verb(false),es_parse(false),subs(0),subs_num(0),silence(false),demux_target(NULL), demux_target2(NULL),pid_to_demux(-1), pid_to_demux2(-1), type_todemux(-1), type_todemux2(-1) {}
