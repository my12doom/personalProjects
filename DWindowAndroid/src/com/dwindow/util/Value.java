	...
	private Value(String Key, T Default) {
		...
		if (db != null) {
			...
			try {
				...
				cursor = db.query(TableName, new String[] { "Value" }, "Key=?",
						new String[] { mKey }, null, null, null);
				if (cursor.moveToFirst()) {
					// mValue=cursor.getString(0);

					if (mObj.getClass() == Float.class)
						mObj = (T) (Object) Float.parseFloat(cursor
								.getString(0));

					else if (mObj.getClass() == Integer.class)
						mObj = (T) (Object) Integer.parseInt(cursor
								.getString(0));

					else if (mObj.getClass() == Boolean.class)
						mObj = (T) (Object) Boolean.parseBoolean(cursor
								.getString(0));
