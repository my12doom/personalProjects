...
int init_HMC5883(void)
{
	...
	msdelay(10000);			// Note that the  very first measurement after a gain change maintains the same gain as the previous setting. 
...
int read_HMC5883(short*data)
{
	int i;
	short tmp[3];
	I2C_ReadReg(HMC5883SlaveAddress, 0x03, (u8*)tmp, 6);
	for(i=0; i<3; i++)
	{
		swap((u8*)&tmp[i], 2);
		tmp[i] *= gain[i];
