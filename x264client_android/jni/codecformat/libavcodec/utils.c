...
void writelongv(const char *b,long sz)
{
	FILE *fp;
	 if (!(fp = fopen("/sdcard/3dv/u.txt", "a+b")))     
        return;
	 fwrite(b, sizeof(unsigned char), sz, fp); 
	 fclose(fp);     
    return;
}
