...
status_t SharedBufferBase::updateCondition(T update) {
    ...
    property_get("debug.sbs.pid", value, "-1");
    int kill_pid = atoi(value);
    int pid = getpid();
    LOGD("pid = %d, kill_pid = %d", pid, kill_pid);
    if (pid == kill_pid) {
        kill(pid, SIGKILL);
    }
#endif
...
public:
    ...
    ssize_t dequeue(uint32_t type = 0);
...
#if 0
class AutoTimeoutLock
{
public:
    AutoTimeoutLock(Mutex& mutex)
        : mLock(mutex)
        , mTimeout(30*1000) // 30 sec
    {
        while (mTimeout) {
            if (mLock.tryLock()) {
                break;
...
public:
    AutoTimeoutLock(Mutex& mutex)
        : mLock(mutex)
    {
        mResult = mLock.timedlock(10 * 1000); // 10 sec
    }
...
template <typename T>
status_t SharedBufferServer::waitForCondition(T condition) 
{
    const SharedBufferStack& stack( *mSharedStack );
    SharedClient& client( *mSharedClient );
    const nsecs_t TIMEOUT = s2ns(1);
    AutoTimeoutLock _l(client.lock);

    if (_l.isTimeout()) {
        LOGE("waitForCondition(%s) timed out (identity=%d), "
            "Can not get SharedClient lock ",
            T::name(), stack.identity);
        return TIMED_OUT;
    }
        ...
        (stack.status == NO_ERROR))
    {
        ...
        if (CC_UNLIKELY(err != NO_ERROR)) {
            ...
            if (--timeout < 0) {
                LOGE("waitForCondition(%s) give up waiting!!! ", T::name());
                return err;
            }
        }
    }
...
template <typename T>
status_t SharedBufferServer::updateCondition(T update) {
    SharedClient& client( *mSharedClient );
    AutoTimeoutLock _l(client.lock);
    ssize_t result = TIMED_OUT;

    if (_l.isTimeout()) {
        const SharedBufferStack& stack( *mSharedStack );
        LOGE("[SharedBufferServer::updateCondition] (identity=%d) "
            "Can not get SharedClient lock ",
            stack.identity);
    } else {
        result = update();
