...
public:
     ...
     */
    /* for OMA DRM v1 feature, you can specify param {offset} and {length} as 0, */
    /* because we don't use these 2 values. */
     ...
     *     Handle for the decryption session
     */
    /* the uri should begin with "file://" and followed by the absolute full path */
    /* file name. */
...
public:
     ...
     * @return String8
     *     Returns content uri for success, empty for failure
     */
    /* modified to return String8 */
    String8 saveRights(
    ...
    /**
     * Install a drm message file with the given protected content
     *
     * @param[in] path Path of the protected content
     * @return status_t
     *     Returns DRM_NO_ERROR for success, DRM_ERROR_UNKNOWN for failure
     */
    status_t installDrmMsg(const String8& path);

    /**
     * Check secure timer status
     *
     * @param[in] path Path of the protected content
     * @return bool
     *     Returns true for valid, false for invalid
     */
    bool checkSecureTimerStatus(const String8& path);
     ...
     *     Returns DRM_NO_ERROR for success, DRM_ERROR_UNKNOWN for failure
     */
    status_t consume(const String8& path, int action);

    /**
     * Open the decrypt session to decrypt the given protected content
     *
     * @param[in] fd File descriptor of the protected content to be decrypted
     * @param[in] offset Start position of the content. not used, can be 0
     * @param[in] length The length of the protected content. not used, can be 0
     * @return
     *     Handle for the decryption session
     */
    DecryptHandle* openDecryptSessionDirect(int fd, int offset, int length);

    ssize_t getContentLength(DecryptHandle* decryptHandle);

    ssize_t getContentSize(DecryptHandle* decryptHandle);

    static String8 getFilePath(int fd);
    ...
    static String8 key_proc_name;

private:
    int mUniqueId;
    Mutex mDecryptLock;
    DrmManagerClientImpl* mDrmManagerClientImpl;
    static KeyedVector<int, DrmManagerClient*> sClientVector;
