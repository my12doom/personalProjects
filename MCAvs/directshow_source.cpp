...
DEFINE_GUID(CLSID_MCDemuxer, 0x79df8a70, 0xe7d5, 0x49db, 0xad, 0xc4, 0xa4, 0xbe, 0x0e, 0x19, 0x3a, 0xa2);
DEFINE_GUID(CLSID_MCMuxer, 0x89a0d875, 0x4073, 0x43be, 0x8e, 0xb5, 0x3c, 0x49, 0xe4, 0xae, 0xe3, 0x70);
DEFINE_GUID(CLSID_MCDecoder, 0x4b3a32c0, 0xea5c, 0x43e9, 0x80, 0xd6, 0x49, 0x28, 0xa0, 0x87, 0xe2, 0xa6);
DEFINE_GUID(CLSID_MCScope, 0xf5a6573b, 0x7393, 0x41fd, 0x80, 0x44, 0x23, 0x85, 0x5a, 0x53, 0x49, 0xcd);
DEFINE_GUID(CLSID_PD10_DEMUXER, 0xF07E981B, 0x0EC4, 0x4665, 0xA6, 0x71, 0xC2, 0x49, 0x55, 0xD1, 0x1A, 0x38);
DEFINE_GUID(CLSID_TMT5Demuxer, 0xd5f9370b, 0xe232, 0x464f, 0xbe, 0x29, 0xa8, 0x85, 0xb1, 0x24, 0x91, 0x90);
/************************************
 *          Logging Utility         *
 ************************************/
...
*/
  ...
  HRESULT __stdcall GetSample::QueryFilterInfo(FILTER_INFO* pInfo) {
    ...
    wcscpy(pInfo->achName, L"GetSample");
...
DirectShowSource::DirectShowSource(const char* filename, const char*filename2, int _avg_time_per_frame, int _seekmode,
...

{
  ...
  try {
	...
	MultiByteToWideChar(CP_ACP, 0, filename2, -1, filename2W, MAX_PATH);
    ...
    if (load_grf) {
      ...
      if (!get_sample.IsConnected()) {
		...
		// filters
		CComPtr<IBaseFilter> source1;
		CComPtr<IBaseFilter> source2;
		CComPtr<IBaseFilter> demuxer1;
		CComPtr<IBaseFilter> demuxer2;
		CComPtr<IBaseFilter> muxer;
		CComPtr<IBaseFilter> decoder;
		CComPtr<IBaseFilter> scope;
		source1.CoCreateInstance(CLSID_AsyncReader);
		source2.CoCreateInstance(CLSID_AsyncReader);
		demuxer2.CoCreateInstance(CLSID_MCDemuxer);
		muxer.CoCreateInstance(CLSID_MCMuxer);
		decoder.CoCreateInstance(CLSID_MCDecoder);
		scope.CoCreateInstance(CLSID_MCScope);
		//CS2SBS * Cconverter = new CS2SBS(_T("S2SBS"), NULL, &hr);
		//CComQIPtr<IBaseFilter, &IID_IBaseFilter> scope(Cconverter);

		// create, add, and load file
		gb->AddFilter(source1, L"Source1");
		gb->AddFilter(source2, L"Source2");
		CComQIPtr<IFileSourceFilter, &IID_IFileSourceFilter> source1f(source1);
		CComQIPtr<IFileSourceFilter, &IID_IFileSourceFilter> source2f(source2);
		hr = source1f->Load(filenameW, NULL);
		CheckHresult(env, hr, "couldn't open file ", filename);
		hr = source2f->Load(filename2W, NULL);
		CheckHresult(env, hr, "couldn't open file ", filename2);

		//demuxer1.CoCreateInstance(CLSID_PD10_DEMUXER);
		//demuxer1.CoCreateInstance(CLSID_TMT5Demuxer);
		if (demuxer1 == NULL)
		{
			printf("using MainConcept Demuxer for file1\n");
			printf("opening file1 %s, this may take one minute or two.\n", filename);
			demuxer1.CoCreateInstance(CLSID_MCDemuxer);
		}
		else
		{
			printf("using Cyberlink Demuxer for file1\n");
		}

		gb->AddFilter(demuxer1, L"Demuxer 1");
		gb->AddFilter(demuxer2, L"Demuxer 2");
		gb->AddFilter(muxer, L"Muxer");
		gb->AddFilter(decoder, L"MC Decoder");
		gb->AddFilter(scope, L"MC Scope");

		// connecte source, demuxer and muxer
		
		// source & demuxer 1
		CComPtr<IPin> source1o;
		CComPtr<IPin> demuxer1i;
		GetUnconnectedPin(source1, PINDIR_OUTPUT, &source1o);
		GetUnconnectedPin(demuxer1, PINDIR_INPUT, &demuxer1i);
		hr = gb->ConnectDirect(source1o, demuxer1i, NULL);
		CheckHresult(env, hr, "couldn't open file ", filename);
		printf("file1 %s opened.\n", filename);

		// source & demuxer 2
		CComPtr<IPin> source2o;
		CComPtr<IPin> demuxer2i;
		GetUnconnectedPin(source2, PINDIR_OUTPUT, &source2o);
		GetUnconnectedPin(demuxer2, PINDIR_INPUT, &demuxer2i);
		hr = gb->ConnectDirect(source2o, demuxer2i, NULL);
		CheckHresult(env, hr, "couldn't open file ", filename2);
		printf("file2 %s opened.\n", filename2);

		// demuxer 1 & muxer
		CComPtr<IPin> demuxer1o;
		CComPtr<IPin> muxeri1;
		GetUnconnectedPin(demuxer1, PINDIR_OUTPUT, &demuxer1o);
		GetUnconnectedPin(muxer, PINDIR_INPUT, &muxeri1);
		hr = gb->ConnectDirect(demuxer1o, muxeri1, NULL);
		CheckHresult(env, hr, "couldn't demux file ", filename);
		printf("file1 %s demuxed.\n", filename);

		// demuxer 2 & muxer
		CComPtr<IPin> demuxer2o;
		CComPtr<IPin> muxeri2;
		GetUnconnectedPin(demuxer2, PINDIR_OUTPUT, &demuxer2o);
		GetUnconnectedPin(muxer, PINDIR_INPUT, &muxeri2);
		hr = gb->ConnectDirect(demuxer2o, muxeri2, NULL);
		CheckHresult(env, hr, "couldn't demux file ", filename2);
		printf("file2 %s demuxed.\n", filename2);

		// muxer & decoder
		CComPtr<IPin> muxero;
		CComPtr<IPin> decoderi;
		GetUnconnectedPin(muxer, PINDIR_OUTPUT, &muxero);
		GetUnconnectedPin(decoder, PINDIR_INPUT, &decoderi);
		hr = gb->ConnectDirect(muxero, decoderi, NULL);
		printf("decoder started.\n");

		// decoder & scope input1
		CComPtr<IPin> decodero1;
		CComPtr<IPin> scopei1;
		GetUnconnectedPin(decoder, PINDIR_OUTPUT, &decodero1);
		GetUnconnectedPin(scope, PINDIR_INPUT, &scopei1);
		hr = gb->ConnectDirect(decodero1, scopei1, NULL);
		printf("scope 1 connected.\n");

		// decoder & scope input2
		CComPtr<IPin> decodero2;
		CComPtr<IPin> scopei2;
		GetUnconnectedPin(decoder, PINDIR_OUTPUT, &decodero2);
		GetUnconnectedPin(scope, PINDIR_INPUT, &scopei2);
		hr = gb->ConnectDirect(decodero2, scopei2, NULL);
		printf("scope 2 connected.\n");

		// scope output
		CComPtr<IPin> scopeo;
		GetUnconnectedPin(scope, PINDIR_OUTPUT, &scopeo);
		gb->Render(scopeo);
    ...
    if (vi.HasVideo()) {
	  ...
	  // my12doom: MainConcept don't support FRAME mode and course deadlock
	  frame_units = false;
...
AVSValue __cdecl Create_DirectShowSource(AVSValue args, void*, IScriptEnvironment* env) {
  ...
  if (args[0].ArraySize() != 2)
    env->ThrowError("DirectShowSource: must 2 input file!");

  const char* filename = args[0][0].AsString();
  const char* filename2 = args[0][1].AsString();
  ...
  if (!(audio && video)) { // Hey - simple!!
    if (audio) {
      return new DirectShowSource(filename, filename2, _avg_time_per_frame, seekmode, true , false,
                                  args[5].AsBool(false), _media, _timeout, _frames, log, env);
    } else {
      return new DirectShowSource(filename, filename2, _avg_time_per_frame, seekmode, false , true,
                                  args[5].AsBool(false), _media, _timeout, _frames, log, env);
    }
  }
  ...
  try {
	...
	try {
	  DS_audio = new DirectShowSource(filename, filename2, _avg_time_per_frame, seekmode, true , false,
									  args[5].AsBool(false), _media, _timeout, _frames, log, env);
	...
	try {
	  DS_video = new DirectShowSource(filename, filename2, _avg_time_per_frame, seekmode, false, true,
									  args[5].AsBool(false), _media, _timeout, _frames, log, env);
	} catch (AvisynthError e) {
