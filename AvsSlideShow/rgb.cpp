...
int ConvertRGBA2YUV(int w,int h,unsigned char *bmp,unsigned char*yuv)
{
	...
	u=uu;
	v=vv;
	// Get r,g,b pointers from bmp image data(BGRA)....
	r=bmp+2;
	g=bmp+1;
	b=bmp;
	...
	for(i=0;i<h;i++)
	{
		for(j=0;j<w;j++)
		{
			*y++= constrain((+0.299) * (*r)  + (+0.587) * (*g) + (+0.114) * (*b));
			*u++= constrain((-0.147) * (*r)  + (-0.289) * (*g) + (+0.436) * (*b) + 128);
			*v++= constrain((+0.615) * (*r)  + (-0.515) * (*g) + (-0.100) * (*b) + 128);
