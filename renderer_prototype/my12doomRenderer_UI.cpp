...
HRESULT ui_drawer_base::uninit(int width, int height,IDirect3DDevice9 *device)
{
...
HRESULT ui_drawer_dwindow::init_gpu(int width, int height, IDirect3DDevice9 *device)
{
	invalidate_gpu();
	HRESULT hr;
	if (!m_ui_logo_gpu)
...
HRESULT ui_drawer_dwindow::init_cpu(int width, int height, IDirect3DDevice9 *device)
{
	m_Device = device;
	m_width = width;
	m_height = height;
...
HRESULT ui_drawer_dwindow::invalidate_gpu()
{
	m_ps_UI = NULL;
	m_vertex = NULL;
	m_ui_logo_gpu = NULL;
...
HRESULT ui_drawer_dwindow::invalidate_cpu()
{
	invalidate_gpu();
	m_ui_logo_cpu = NULL;
	m_ui_tex_cpu = NULL;
...
HRESULT ui_drawer_dwindow::draw_ui(IDirect3DSurface9 * surface, REFERENCE_TIME current, REFERENCE_TIME total, float volume, bool running, float alpha)
{
	if (!surface)
		return E_POINTER;
...
HRESULT ui_drawer_dwindow::draw_nonmovie_bg(IDirect3DSurface9 *surface, bool left_eye)
{
