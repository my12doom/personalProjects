	...
	else
    {
        ...
        if(s.type!=0xff)
        {
            ...
            if(s.frame_num)
            {
				...
				if (demux_target && (pid == pid_to_demux || s.type == type_todemux))
					demux_target->insert(len, (BYTE*)ptr);
...
//my12doom
int ts::demuxer::fast_scan_file(const char* name, int scan_packet_count/*=50000*/)
{
	// some init
	prefix.clear();
	...
	// detect stream type;
	DWORD read;
	HANDLE f = CreateFile(name, FILE_READ_ACCESS, 7, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	// scan last packets
	LONGLONG packet_count = filesize.QuadPart/buf_len;
	LARGE_INTEGER target_pos;
	target_pos.QuadPart = (packet_count-scan_packet_count)*buf_len;
...
int ts::demuxer::demux_file(const char* name)
{
    ...
    for(u_int64_t pn=1;;pn++)
    {
        ...
        }else
        {
            ...
            {
                buf_len=188;
                if(!silence)fprintf(stderr,"TS stream detected in %s (packet length=%i)\n",name,buf_len);
                hdmv=false;
            }else if(buf[0]!=0x47 && buf[4]==0x47)
            {
                if(file.read(buf+188,4)!=4)
                    break;
                buf_len=192;
                if(!silence)fprintf(stderr,"M2TS stream detected in %s (packet length=%i)\n",name,buf_len);
                hdmv=true;
            }else
            {
                if(!silence)fprintf(stderr,"unknown stream type in %s\n",name);
                return -1;
            }
        }
