...
namespace ts
{
    ...
    public:
		...
		bool silence;
		int fast_scan_file(const char* name, int scan_packet_count=50000);
...
#endif
    public:
        demuxer(void):hdmv(false),av_only(true),parse_only(false),dump(0),channel(0),base_pts(0),pes_output(0),verb(false),es_parse(false),subs(0),subs_num(0),silence(false),demux_target(NULL),pid_to_demux(-1), type_todemux(-1) {}
