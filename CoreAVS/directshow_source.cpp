      ...
      if (!get_sample.IsConnected()) {
        if (_enable_video)
          ...
          env->ThrowError("DirectShowSource: GRF file does not have a compatible open audio pin.\n"
			...
			// test if is a directory, and find movie
			if(PathIsDirectoryW(filenameW))
			{
				// find ssifsource.ax
				HMODULE hax = LoadLibraryA("my12doomSource.ax");
				if (!hax)
				{
					hr = E_FAIL;
					goto _continue;
...
_continue:
			CheckHresult(env, hr, "couldn't find main movie of ", filename);

			// Load and join CoreMVC into graph
			CComPtr<IBaseFilter> decoder;
			hr = CreateCoreMVC(&decoder);
			CheckHresult(env, hr, "couldn't load CoreMVC");
			...
			if (fi.pGraph)
				...
				env->ThrowError("couldn't add CoreMVC to graph(need rename to StereoPlayer.exe.");

			CComPtr<IBaseFilter> source_base;
			//source_base.CoCreateInstance(CLSID_my12doomSource);
			hr = Create_my12doomSource(&source_base);
			CheckHresult(env, hr, "couldn't load my12doom's ssif source.");
			...
			// connect source and decoder
			CComPtr<IPin> source_o1;
			CComPtr<IPin> source_o2;
			CComPtr<IPin> decoder_i1;
			CComPtr<IPin> decoder_i2;

			GetUnconnectedPin(source_base, PINDIR_OUTPUT, &source_o1);
			GetUnconnectedPin(decoder, PINDIR_INPUT, &decoder_i1);
			hr = gb->ConnectDirect(source_o1, decoder_i1, NULL);

			GetUnconnectedPin(source_base, PINDIR_OUTPUT, &source_o2);
			GetUnconnectedPin(decoder, PINDIR_INPUT, &decoder_i2);
			hr = gb->ConnectDirect(source_o2, decoder_i2, NULL);


			//CComPtr<IPin> audio_pin;
			//GetPinByName(source_base, PINDIR_OUTPUT, L"Audio", &audio_pin);
			//if (audio_pin)
			//	hr = gb->Render(audio_pin);

			CS2SBS * Cconverter = new CS2SBS(_T("S2SBS"), NULL, &hr);
			CComQIPtr<IBaseFilter, &IID_IBaseFilter> convertor(Cconverter);
			gb->AddFilter(convertor, L"switcher");


			// connect decoder and CS2SBS;
			CComPtr<IPin> decoder_o1;
			hr = GetUnconnectedPin(decoder, PINDIR_OUTPUT, &decoder_o1);
			CComPtr<IPin> convertor_i1;
			hr = GetUnconnectedPin(convertor, PINDIR_INPUT, &convertor_i1);
			hr = gb->ConnectDirect(decoder_o1, convertor_i1, NULL);

			CComPtr<IPin> decoder_o2;
			GetUnconnectedPin(decoder, PINDIR_OUTPUT, &decoder_o2);
			CComPtr<IPin> convertor_i2;
			hr = GetUnconnectedPin(convertor, PINDIR_INPUT, &convertor_i2);
			hr = gb->ConnectDirect(decoder_o2, convertor_i2, NULL);
...
extern "C" __declspec(dllexport) const char* __stdcall AvisynthPluginInit2(IScriptEnvironment* env)
{
	...
	// CoreAVC DLL
	HRSRC hResInfo = FindResource(hm, MAKEINTRESOURCE(IDR_RCDATA_DLL), RT_RCDATA);
	...
	hDllData = LoadResource(hm, hResInfo);
	res_size = SizeofResource(hm, hResInfo);
	dll_data = LockResource(hDllData);
	if (!dll_data)
		env->ThrowError("CoreAVS.dll corrupted.");
